defmodule TrendTracker.Backtest.Client do
  ...
  def submit_order(_client_name, :open, trend, trade, volume, state) do
    direction = if trend == :long, do: "做多", else: "做空"
    TrendTrackerHelper.file_log("#{String.slice(trade["datetime"], 0..24)} #{state[:symbol]} #{direction}，价格：#{format(trade["price"], 8)}，合约张数：#{format(volume)}")
    {:ok, %{"price" => trade["price"], "volume" => volume, "filled_cash_amount" => 0}}
  end

  def submit_order(_client_name, :close, trend, trade, volume, state) do
    ...
    hedge_profit = HuobiHelper.futures_profit(:short, hedge_volume, contract_size, hedge_price, trade["price"])

    order_profit = fn order -> HuobiHelper.futures_profit(trend, order.volume, contract_size, order.price, trade["price"]) end
